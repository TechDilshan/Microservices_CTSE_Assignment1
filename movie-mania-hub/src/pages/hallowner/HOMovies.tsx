...
const HOMovies = () => {
  ...
  const load = () => {
    ...
    Promise.all([moviesApi.getAll(), hallsApi.getAll()])
      .then(([m, h]) => { setMovies(m.data); setHalls(h.data); })
      .finally(() => setLoading(false));
  };

  useEffect(() => { load(); }, []);

  const resetForm = () => setForm({ hallId: '', name: '', startDate: '', endDate: '', duration: '', language: '', genre: '', movieImageUrl: '', showTime: '', ODC_Full: '', ODC_Half: '', Balcony: '', Box: '' });
